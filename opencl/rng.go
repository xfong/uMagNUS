...
func (g *Generator) Init(seed *uint64) {
	...
	if seed == nil {
		g.PRNG.Init(initRNG(), ClCmdQueue[0], nil)
	} else {
		g.PRNG.Init(*seed, ClCmdQueue[0], nil)
	}
	if g.buf == nil {
...
func (g *Generator) Uniform(data unsafe.Pointer, d_size int) {
	...
	for demand > 0 {
		if g.supply <= 0 {
			event = g.PRNG.GenerateUniform(g.buf.DevPtr(0), g.buf_size, ClCmdQueue[0], nil)
		...
		if g.supply >= demand {
			tmpEvent, err = ClCmdQueue[0].EnqueueCopyBuffer((*cl.MemObject)(g.buf.DevPtr(0)), (*cl.MemObject)(data), SIZEOF_FLOAT32*g.sup_offset, SIZEOF_FLOAT32*demand_offset, SIZEOF_FLOAT32*demand, nil)
			if err != nil {
			...
			tmpEvent, err = ClCmdQueue[0].EnqueueCopyBuffer((*cl.MemObject)(g.buf.DevPtr(0)), (*cl.MemObject)(data), SIZEOF_FLOAT32*g.sup_offset, SIZEOF_FLOAT32*demand_offset, SIZEOF_FLOAT32*g.supply, nil)
			if err != nil {
				fmt.Printf("EnqueueCopyBuffer in copying uniform random numbers failed: %+v \n", err)
			}
			if err = cl.WaitForEvents([]*cl.Event{tmpEvent}); err != nil {
...
func (g *Generator) Normal(data unsafe.Pointer, d_size int) {
	...
	for demand > 0 {
		if g.supply <= 0 {
			event = g.PRNG.GenerateNormal(g.buf.DevPtr(0), g.buf_size, ClCmdQueue[0], nil)

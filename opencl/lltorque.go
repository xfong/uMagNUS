...
func LLTorque(torque, m, B *data.Slice, alpha MSlice) {
	...
	ClLastEvent = k_lltorque2_async(torque.DevPtr(X), torque.DevPtr(Y), torque.DevPtr(Z),
	...
	if err = ClCmdQueue[0].Flush(); err != nil {
		log.Printf("failed to flush queue in lltorque: %+v \n", err)
	}
...
func LLNoPrecess(torque, m, B *data.Slice) {
	...
	ClLastEvent = k_llnoprecess_async(torque.DevPtr(X), torque.DevPtr(Y), torque.DevPtr(Z),
		m.DevPtr(X), m.DevPtr(Y), m.DevPtr(Z),
		B.DevPtr(X), B.DevPtr(Y), B.DevPtr(Z), N, cfg,
		ClCmdQueue[0], tmpEvents)

	if err = ClCmdQueue[0].Flush(); err != nil {
		log.Printf("failed to flush queue in llnoprecess: %+v \n", err)
	}
